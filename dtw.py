    ...
    for digit in range(10):
        ...
        for j in range(len(file_list)):
            ...
            if file_path[-4:] == ".wav":
                try:
                    # 使用新的特征提取函数
                    wav_mfcc = extract_mfcc_for_training(file_path)
...
def predict_dtw(model, file_path):
    ...
    try:
        # 使用新的特征提取方法
        mfcc_feat = extract_mfcc_for_training(file_path)
...
# def train_model_dtw(train_dir): # 使用原有特征提取方法的训练函数
#     """
#     :param train_dir: directory of train audios
#     :return: the trained mfcc model
#     """
...
# def predict_dtw(model, file_path): # 使用原有特征提取方法的预测函数
#     """
#     :param model: trained model
#     :param file_path: path of .wav file
#     :return: digit
...
if __name__ == '__main__':
    ...
    model = train_model_dtw(train_dir) # prepocess已经包含端点检测，不需要在这里重复检测
    count = 0
    pred_true = 0
    for i in range(10):
